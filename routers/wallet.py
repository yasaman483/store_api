...
async def add_(added_amount: UpdateAmount, current_user: People = Depends(auth.get_current_user), db: AsyncSession = Depends(get_db)):
    return await update_wallet_amount(added_amount, current_user, db)


@router.post('/get_remain_wallet_amount')
async def recieve_wallet_amount(current_user: People = Depends(auth.get_current_user), db: AsyncSession = Depends(get_db)):
