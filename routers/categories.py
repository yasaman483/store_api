...
async def add_catgory(category: CategoryCreate, current_user: People = Depends(auth.get_current_user), db: AsyncSession = Depends(get_db)):
    return await create_category(category, current_user, db)


@router.post('/get_categories', response_model=list[CategoryResponse])
async def read_all_categories(current_user: People = Depends(auth.get_current_user), db: AsyncSession = Depends(get_db)):
    return await get_all_categories(current_user, db)


@router.post('/get_category', response_model=CategoryResponse)
async def read_category_by_id(category: GetCategoryId, current_user: People = Depends(auth.get_current_user), db: AsyncSession = Depends(get_db)):
    return await get_category_by_id(category, current_user, db)


@router.put('/edit_category', response_model=CategoryResponse)
async def edit_category(current_categor: CategoryBase, category: CategoryCreate, current_user: People = Depends(auth.get_current_user), db: AsyncSession = Depends(get_db)):
    return await update_category(current_categor, category, current_user, db)


@router.delete('/delete_category')
async def remove_category(category: CategoryBase, current_user: People = Depends(auth.get_current_user), db: AsyncSession = Depends(get_db)):
